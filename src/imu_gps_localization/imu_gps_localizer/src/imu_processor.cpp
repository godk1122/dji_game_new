...
namespace ImuGpsLocalization
{
    ...
    { // 4.1 利用IMU数据进行状态预测；式 (269)（270）（268）
        ...
        // Acc and gyro.
        const Eigen::Vector3d acc_unbias = 0.5 * (last_imu->acc + cur_imu->acc) - last_state.acc_bias;     //  a_m - a_b; 上一时刻和当前时刻的IMU数据均值 - 对应的bias，作为真实IMU数据；
        const Eigen::Vector3d gyro_unbias = 0.5 * (last_imu->gyro + cur_imu->gyro) - last_state.gyro_bias; //  w_m - w_b;
        ...
        Eigen::Matrix<double, 15, 15> Fx = Eigen::Matrix<double, 15, 15>::Identity(); // P59，式 (269);
        ...
        Eigen::Matrix<double, 15, 12> Fi = Eigen::Matrix<double, 15, 12>::Zero(); // P60，式(270)
        ...
        state->cov = Fx * last_state.cov * Fx.transpose() + Fi * Qi * Fi.transpose(); // P59，式(268);
