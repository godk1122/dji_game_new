...
namespace ImuGpsLocalization
{
    ...
    { // 3.2 添加GPS数据，并初始化系统状态（尤其是根据加速度估算的初始姿态）；
        ...
        { // 3.2.2 当最新的IMU队列中数据足够，计算加速度的均值，从而估算处初始的IMU姿态（注意，在初始化的这段时间内，载体必须静止）；
        ...
        // Set covariance.
        state->cov.setZero();
        state->cov.block<3, 3>(0, 0) = 0.001 * Eigen::Matrix3d::Identity(); // position std: 10 m
        state->cov.block<3, 3>(3, 3) = 0.001 * Eigen::Matrix3d::Identity(); // velocity std: 10 m/s
        // roll pitch std 10 degree.
        state->cov.block<2, 2>(6, 6) = 0.01 * kDegreeToRadian * 0.01 * kDegreeToRadian * Eigen::Matrix2d::Identity();
        state->cov(8, 8) = 0.01 * kDegreeToRadian * 0.01 * kDegreeToRadian; // yaw std: 100 degree.
        // Acc bias.
    ...
    { // 3.2.2
        ...
        const Eigen::Vector3d std_acc = (sum_err2 / (double)imu_buffer_.size()).cwiseSqrt(); // 计算加速度的标准差

        if (std_acc.maxCoeff() > kAccStdLimit)
        { // 3.2.2.1 如果初始化这段时间的载体没有静止，那么这段时间测量出的加速度的标准差会比较大，此时初始化会失败，预测初始化时一定要载体静止；
        ...
        // z-axis.
        // 3.2.2.2 这一段是重点，根据初始化过程中计算的IMU平均加速度，推出IMU的初始姿态，实际上就是全局坐标系到IMU坐标系的旋转矩阵；
        const Eigen::Vector3d &z_axis = -mean_acc.normalized(); // 平均加速度的单位化
