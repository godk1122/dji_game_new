...
class Traj():
    ...
    def sample(self, pos, dt, N):
        ...
        if min_idx == 0:
            # print("min_idx == 0")
            # print("self._u_dir[0]:",self._u_dir[0])
            # print("self._dir_norm:",self._dir_norm[0])
            idx0 = min_idx
        ...
        # print("t0:", t0)
        # print("poss:",self._poss)
        # print("yaws:",self._yaws)
        # print("ts",self._ts)
        # print(ts)
...
x_0, y_0, z_0, vx_0, vy_0, vz_0 = 0, 0, 0, 0, 0, 0
def cal_v(x, dt):
    #
    global x_0, y_0, z_0, vx_0, vy_0, vz_0

    alpha = 0.45  # Filter coefficient, 0 < alpha < 1

    vx = alpha * ((x.x - x_0)/dt) + (1 - alpha) * vx_0
    vy = alpha * ((x.y - y_0)/dt) + (1 - alpha) * vy_0
    vz = alpha * ((x.z - z_0)/dt) + (1 - alpha) * vz_0
...
def odom_cb(msg: PoseStamped):
    ...
    # calculate v
    vx ,vy ,vz = cal_v(msg.pose.position, t_control)
    # 
    if trajectory != None:
        ...
        u.pitchRate = wy
        u.yawRate = wz
        u.throttle = Tt/quad._a_z_max 
        setpoint_raw_pub.publish(u)
        t_end = time.time()
        t_control = t_end - t_start 
        print("publish##############################################")
